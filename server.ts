...
async function startServer() {
  ...
  // API: Proxy for AI Providers to avoid CORS
  app.post("/api/proxy/ai", async (req, res) => {
    ...
    try {
      const response = await axios({
        ...
        headers: {
          ...headers,
          "User-Agent": "AI-Video-Studio/1.0.0"
        },
        data
      });
      ...
      console.error(`[Proxy Error] ${url} (${status}):`, typeof errorData === 'string' ? errorData.substring(0, 200) : errorData || error.message);
      
      // Always return a structured JSON error to prevent frontend parsing errors
      res.status(status).json({
        error: {
          message: typeof errorData === 'string' && errorData.includes("<!DOCTYPE") 
            ? `API returned an HTML error page. This usually means the endpoint is invalid or the service is down. (Status: ${status})`
            : (errorData?.error?.message || errorData || error.message),
          status,
          raw: typeof errorData === 'string' ? errorData.substring(0, 100) : errorData
        }
      });
    }
  });
  ...
  app.get("/api/stats", async (req, res) => {
    try {
      ...
      const loadAvg = (loadAvgArr && loadAvgArr.length > 0) ? loadAvgArr[0] : 0;
      const cpuLoad = Math.min(Math.round((loadAvg / Math.max(cpuCount, 1)) * 100), 100) || 0;
      ...
      try {
        ...
        if (diskSpace && diskSpace.size > 0) {
          storageUsage = Math.round(((diskSpace.size - diskSpace.free) / diskSpace.size) * 100);
          totalStorageGB = Math.round(diskSpace.size / (1024 * 1024 * 1024));
        }
      } catch (diskErr) {
        console.error("Error checking disk space:", diskErr);
      }
      ...
      res.json({
        ...
        totalRam: Math.round(totalMem / (1024 * 1024 * 1024)) || 0,
        storage: isNaN(storageUsage) ? 0 : storageUsage,
        totalStorage: totalStorageGB,
        videoSchedule: db.jobs.filter(j => j && j.status && !['completed', 'failed'].includes(j.status.toLowerCase())).length,
        activeChannels: db.channels.length,
        queueSize: db.jobs.filter(j => j && j.status && !['completed', 'failed'].includes(j.status.toLowerCase())).length
      });
    } catch (error) {
      console.error("Error fetching system stats:", error);
      res.status(500).json({ error: "Failed to fetch system stats", details: error instanceof Error ? error.message : String(error) });
    }
  });
